...
class ComparisonReporter:
    ...
    def generate_report(self, run_results: list[dict[str, Any]]) -> str:
        ...
        sorted_runs = sorted(
            run_results,
            key=lambda r: (
                r["metrics"].get("ndcg_at_10", 0.0) - (r["metrics"].get("avg_latency_ms", 0.0) / 1000.0)
            ),
            reverse=True,
        )
        ...
        report_lines = [
            ...
            f"- **Embed Ingestion Latency**: `{round(top_run['metrics'].get('embed_time_ms', 0.0), 2)} ms`",
            "",
            "## Matrix Benchmark Comparison Table",
            "",
            "| Run Name | Embedding | Orchestration | MRR@10 | NDCG@10 | Avg Latency (ms) | Embed Time (ms) | Index Time (ms) |",
            "|---|---|---|---|---|---|---|---|",
        ]
        ...
        for r in sorted_runs:
            ...
            row = f"| `{name}` | `{emb}` | `{orch}` | `{mrr}` | `{ndcg}` | `{lat}` | `{embed_t}` | `{index_t}` |"
